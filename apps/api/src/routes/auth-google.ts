...
export function authGoogleRoutes(deps: AppDeps): Hono<CoreAuthHonoEnv> {
  ...
  app.get(CALLBACK_PATH, async (c) => {
    ...
    try {
      ...
      // Hallazgo de auditoría (P2, "tokens de sesión completos en query params
      // de URL") -- ver el comentario de cabecera de `packages/db/migrations/
      // 0008_auth_exchange_code.sql`. NUNCA se pone el JWT real en esta URL --
      // solo un código de intercambio opaco de un solo uso y 60s de vida, que
      // `GoogleCallback.tsx` canjea de inmediato vía `POST /auth/exchange-code`
      // por `{token, refreshToken}` en el BODY de una respuesta JSON.
