...
/**
 * Correo real vía `despachos.messaging_outbox` con el enlace de activación ya
 * armado — best-effort: un fallo al encolar el correo NUNCA debe revertir la
 * invitación que sí quedó creada (`createStaffInvite`, ya persistida por el
 * caller ANTES de invocar esto, misma sesión de staff).
 *
 * SAVEPOINT (corrección de revisión sobre PR #176, mismo patrón que
 * `vencimientos/email-notifications.ts::tryEnqueueEscalationEmail`): esta
 * ruta corre en la sesión de STAFF del request, en la MISMA transacción que
 * ya persistió `createStaffInvite`. Sin `runWithRowSavepoint`, un error real
 * de Postgres en el encolado (deadlock/timeout transitorio, o `42501` del
 ...
 * `PostgresDespachosRepository` + `AbortAwareFakeSession` sin pasar por la
 * ruta HTTP completa (ver `despachos-admin-staff-savepoint.spec.ts`).
 */
...
export function despachosAdminStaffRoutes(deps: AppDeps): Hono<CoreAuthHonoEnv> {
  ...
  app.post(collectionPath, async (c) => {
    ...
    await tryEnqueueStaffInviteEmail(deps.despachosRepo(c.get("db")), organizationId, invite.id, email, correo);
