...
export function citasVoiceToolsRoutes(deps: AppDeps): Hono {
  ...
  app.post("/v1/citas/:orgSlug/availability", async (c) => {
    ...
    const limited = await consumeRateLimit(deps.citasRepo, "voice-availability", requestActor(c.req.raw, providerId), 120, 60);
    if (!limited.allowed) throw Errors.tooManyRequests();

    try {
      const { slots } = await queryAvailability(deps.citasRepo, { organizationId: org.id, providerId, serviceId, dateStr: date });
  ...
  app.post("/v1/citas/:orgSlug/services", async (c) => {
    ...
    const limited = await consumeRateLimit(deps.citasRepo, "voice-services", requestActor(c.req.raw), 120, 60);
    if (!limited.allowed) throw Errors.tooManyRequests();

    const services = await deps.citasRepo.listActiveServices(org.id);
  ...
  app.post("/v1/citas/:orgSlug/providers", async (c) => {
    ...
    const limited = await consumeRateLimit(deps.citasRepo, "voice-providers", requestActor(c.req.raw, serviceId ?? ""), 120, 60);
    if (!limited.allowed) throw Errors.tooManyRequests();

    const providers = await deps.citasRepo.listActiveProviders(org.id, serviceId);
  ...
  app.post("/v1/citas/:orgSlug/customers/appointments", async (c) => {
    ...
    const limited = await consumeRateLimit(deps.citasRepo, "voice-customer-appointments", requestActor(c.req.raw, customerPhone), 30, 60);
    if (!limited.allowed) throw Errors.tooManyRequests();

    const { appointments } = await findAppointmentsForCustomerPhone(deps.citasRepo, org.id, customerPhone);
