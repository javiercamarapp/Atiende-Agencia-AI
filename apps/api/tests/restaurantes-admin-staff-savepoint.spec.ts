...
// `tryEnqueueStaffInviteEmail` (apps/api/src/routes/verticals/restaurantes/admin-staff.ts).
...
//
// Se prueba contra `PostgresRestaurantesRepository` real (no el doble en
...
// `restaurantes-email-dispatch-savepoint.spec.ts` de este mismo directorio).
// Cada test de "fallo" de este archivo FALLA contra el código anterior
// (try/catch sin `repo.runWithRowSavepoint`): sin el SAVEPOINT, la consulta
// posterior sobre la MISMA sesión lanza 25P02 en vez de resolver.
import { describe, expect, it, vi } from "vitest";
import type { TenantDbSession } from "@atiende/core-tenancy";
import { PostgresRestaurantesRepository } from "@atiende/domain-restaurantes";
import { tryEnqueueStaffInviteEmail } from "../src/routes/verticals/restaurantes/admin-staff.ts";
...
describe("tryEnqueueStaffInviteEmail (restaurantes) — SAVEPOINT (corrección de revisión PR #176)", () => {
