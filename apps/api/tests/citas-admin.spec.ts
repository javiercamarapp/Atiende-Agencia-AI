...
describe("GET /v1/citas/properties/:propertyId/providers(/:providerId)", () => {
  ...
  it("la ficha de un proveedor trae también el estado de Cal.com y CalDAV, sin secretos", async () => {
    const ctx = await buildCitasTestContext(buildApp);
    const app = buildApp(ctx.deps);
    await ctx.citasRepo.connectProviderCalComAccount({ organizationId: ctx.organizationId, providerId: ctx.providerId, calcomEventTypeId: "555", apiKey: "llave-ficticia-calcom" });
    await ctx.citasRepo.connectProviderCalDavAccount({ organizationId: ctx.organizationId, providerId: ctx.providerId, calendarCollectionUrl: "https://caldav.example.com/x/", username: "x@y.com", password: "clave-ficticia-caldav" });
    ...
    expect(JSON.stringify(body)).not.toContain("llave-ficticia-calcom");
    expect(JSON.stringify(body)).not.toContain("clave-ficticia-caldav");
  });
