...
describe("triggerCitasEmailDispatchInline — SAVEPOINT (regresión auditoría a2)", () => {
  it("con sesión de staff (claimEmailOutboxBatch lanza 42501): SAVEPOINT -> ROLLBACK TO SAVEPOINT -> RELEASE, nunca relanza", async () => {
    ...
    await expect(triggerCitasEmailDispatchInline(ctx.deps, session, repo)).resolves.toBeUndefined();
  ...
  it("éxito real: SAVEPOINT -> RELEASE, sin ROLLBACK TO SAVEPOINT", async () => {
    ...
    await triggerCitasEmailDispatchInline(ctx.deps, session, repo);
