...
import { fetchAuditoria } from "../src/verticals/restaurantes/lib/auditoria-client.ts";

describe("fetchAuditoria (restaurantes)", () => {
  it("pide GET .../admin/auditoria sin ningún filtro cuando no se pasa ninguno", async () => {
    const fetchImpl = vi.fn(async (url: string) => {
      expect(url).toBe("http://api.local/v1/restaurantes/property-1/admin/auditoria");
  ...
  it("arma el query string con tipo/desde/hasta/limit/offset cuando vienen presentes", async () => {
    const fetchImpl = vi.fn(async (url: string) => {
      ...
      expect(parsed.pathname).toBe("/v1/restaurantes/property-1/admin/auditoria");
      expect(parsed.searchParams.get("tipo")).toBe("producto");
    ...
    await fetchAuditoria(fetchImpl, "http://api.local", "tok", "property-1", { tipo: "producto", desde: "2026-01-01", hasta: "2026-01-31", limit: 25, offset: 25 });
