...
import { claveFicticia, llaveFicticia } from "./test-utils/credenciales-ficticias.ts";
...
describe("connectCalCom", () => {
  it("POST con api_key/event_type_id (sin base_url) y mapea la respuesta, sin exponer el api_key", async () => {
    const fetchImpl = vi.fn(async (url: string, init?: RequestInit) => {
      ...
      expect(init?.method).toBe("POST");
      expect(JSON.parse(init!.body as string)).toEqual({ api_key: llaveFicticia("123"), event_type_id: "555" });
  ...
  it("con base_url (self-hosted) lo manda en el body y lo mapea de vuelta", async () => {
    const fetchImpl = vi.fn(async (_url: string, init?: RequestInit) => {
      expect(JSON.parse(init!.body as string)).toEqual({ api_key: llaveFicticia("123"), event_type_id: "555", base_url: "https://calcom.miempresa.example/v2" });
    ...
    const result = await connectCalCom(fetchImpl, "http://api.local", "tok", "prop-1", "prov-1", { apiKey: llaveFicticia("123"), eventTypeId: "555", baseUrl: "https://calcom.miempresa.example/v2" });
...
describe("connectCalDav / disconnectCalDav / fetchCalDavStatus / testCalDavConnection", () => {
  it("connectCalDav hace POST con calendar_collection_url/username/password reales", async () => {
    const fetchImpl = vi.fn(async (url: string, init?: RequestInit) => {
      ...
      expect(JSON.parse(init!.body as string)).toEqual({ calendar_collection_url: "https://caldav.example.test/dav/calendars/user/x@y.com/abc/", username: "x@y.com", password: claveFicticia("real") });
      return new Response(JSON.stringify({ connected: true, provider_id: "prov-1", calendar_collection_url: "https://caldav.example.test/dav/calendars/user/x@y.com/abc/", username: "x@y.com", sync_status: "connected" }), { status: 200 });
    }) as unknown as typeof fetch;
