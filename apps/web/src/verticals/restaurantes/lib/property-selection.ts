// Persistencia de la sucursal activa en el panel de restaurantes — cierra el
...
// fijaba `propertyId` a `branches[0]!.propertyId` sin selector ni persistencia, así
// que una cadena con más de una sucursal jamás podía operar la segunda en adelante
// desde el panel.
//
// MISMO patrón EXACTO que ya resolvió este problema en hoteles/despachos/rentas (ver
// apps/web/src/verticals/hoteles/lib/property-selection.ts, usado aquí literalmente
// como referencia) — namespaced por organización, misma razón: dos organizaciones de
// restaurantes abiertas en el mismo navegador (u otra vertical, otra pestaña) no
// deben pisarse la selección. La resolución de "qué sucursal queda activa dado lo
// persistido" vive en `resolveActivePropertyId` (../dashboard-client.ts) — este
// archivo NO la duplica, solo agrega el read/write de storage.
//
// Separado de RestaurantesShell.tsx a propósito, mismo motivo que
// dashboard-client.ts: probarlo con vitest en entorno "node" (sin DOM) mientras el
// componente solo conecta esta lógica a estado/render real.
import type { SessionStorageLike } from "../../../lib/auth-client.ts";

const KEY_PREFIX = "atiende.restaurantes.selectedProperty.";
 ...
 * RestaurantesShell.tsx durante la sesión actual del tab — solo no sobrevive a
