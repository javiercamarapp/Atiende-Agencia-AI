...
// código real que ni siquiera restaurantes podía hacer esto — todo lo de arriba
// (createStaffInvite/fetchStaffInvites/revokeStaffInvite) solo fija el rol AL
// INVITAR, nunca después. `OrgMember` (a diferencia de `RepartidorMember`, Fase 12,
// deliberadamente sin rol) SÍ trae `verticalRole` — es el dato que la tabla nueva
// "Staff activo" necesita mostrar/editar.
...
export async function fetchOrgMembers(fetchImpl: typeof fetch, apiBaseUrl: string, token: string, propertyId: string): Promise<readonly OrgMember[]> {
  const body = await fetchJson<{ miembros: OrgMember[] }>(fetchImpl, `${apiBaseUrl}/v1/restaurantes/${propertyId}/admin/staff/miembros`, token);
