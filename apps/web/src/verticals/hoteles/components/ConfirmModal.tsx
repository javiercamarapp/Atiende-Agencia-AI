...
export function ConfirmModal({ open, title, message, confirmLabel, cancelLabel = "Volver", busy = false, onConfirm, onCancel }: ConfirmModalProps) {
  ...
  return (
    <div role="presentation" style={overlayStyle} onClick={() => !busy && onCancel()}>
      <div role="alertdialog" aria-modal="true" aria-labelledby="hoteles-confirm-modal-title" style={dialogStyle} onClick={(e) => e.stopPropagation()}>
        <h2 id="hoteles-confirm-modal-title" style={{ margin: "0 0 8px", fontSize: 16 }}>
          {title}
        </h2>
        <p style={{ margin: "0 0 18px", fontSize: 13, color: "#4b5563" }}>{message}</p>
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
          <button type="button" onClick={onCancel} disabled={busy} style={{ padding: "8px 14px", borderRadius: 8, border: "1px solid #d1d5db", background: "#fff", color: "#111827", fontSize: 13, cursor: "pointer" }}>
            {cancelLabel}
          </button>
          <button
