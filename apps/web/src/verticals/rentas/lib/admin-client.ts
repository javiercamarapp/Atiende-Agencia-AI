...
export async function fetchJson<T>(fetchImpl: typeof fetch, url: string, token: string, authCtx: AuthedFetchContext<LoginSession> = defaultAuthCtx()): Promise<T> {
  ...
  if (!res.ok) {
    throw new RentasAdminError(await readErrorMessage(res, `No se pudo cargar ${url} (${res.status}).`));
  }
...
export async function sendJson<T>(
  ...
  if (!res.ok) {
    throw new RentasAdminError(await readWriteErrorMessage(res, `No se pudo completar la solicitud a ${url} (${res.status}).`));
  }
  return (await res.json()) as T;
}
