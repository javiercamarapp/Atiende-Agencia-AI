...
-- (leer ese header primero para el diagnóstico completo): `hoteles.
-- enqueue_messaging_outbox` (migración 008) y `hoteles.claim_email_outbox_batch`/
-- `complete_email_outbox_job` (migración 014) son `security definer` pero el GRANT
...
-- "permission denied for function..." sin importar quién llame.
--
-- Verificado con `grep -rn` sobre `apps/api/src/routes/verticals/hoteles/` antes de
...
--   - `enqueue_messaging_outbox`: se llama SIEMPRE desde rutas de staff YA
--     autenticado (`reservas.ts`/`cfdi.ts`/`folios.ts`, todas con
--     `requirePropertyMembership("propertyId")` + `c.get("db")` real) -- no se
--     encontró ningún caller bajo la sesión de sistema para hoteles, pero la
...
--     encolado desde un cron/webhook sin sesión de staff, sin necesitar otra
--     migración. La verificación real hoy es property-scoped
--     (`core.has_property_access`, MISMA función que ya usan todas las policies
--     RLS de este esquema) -- más estricta que un simple membership de
--     organización, porque `hoteles.messaging_outbox` particiona por
--     `property_id`, no por `organization_id` (ver comentario de migración 008).
...
) returns void language plpgsql security definer set search_path = hoteles as $$
begin
  if auth.uid() is not null then
...
grant execute on function hoteles.enqueue_messaging_outbox(uuid, uuid, text, text, text, jsonb) to authenticated;
grant execute on function hoteles.claim_email_outbox_batch(integer) to authenticated;
grant execute on function hoteles.complete_email_outbox_job(uuid, text, text) to authenticated;
