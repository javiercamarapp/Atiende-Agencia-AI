...
-- mismo bug raíz que ya cerraron 012_email_outbox_authenticated_grants.sql
-- (outbox de correo) y 013_rpc_anti_duplicado_authenticated_grants.sql
-- (whatsapp inbound/rate-limit/create_order_idempotent) --
...
-- `service_role` contra Postgres real.
--
-- `claim_messaging_outbox_batch`/`complete_messaging_outbox_{sent,retry,dead}`
-- (migrations/007_messaging_outbox.sql, el dispatcher de WhatsApp SALIENTE)
...
-- llamador único que las versiones de citas/hoteles (`apps/api/src/routes/
-- internal/whatsapp-dispatch.ts`, ruta de PLATAFORMA gateada por secreto
-- compartido, SIEMPRE `deps.engine.withAppSession({ userId: null }, ...)`) --
-- verificación interna "auth.uid() is null" a secas, mismo criterio ya usado en
-- 013_rpc_anti_duplicado_authenticated_grants.sql.
...
grant execute on function restaurantes.claim_messaging_outbox_batch(integer, integer) to authenticated;
grant execute on function restaurantes.complete_messaging_outbox_sent(uuid) to authenticated;
grant execute on function restaurantes.complete_messaging_outbox_retry(uuid, integer, text, timestamptz) to authenticated;
grant execute on function restaurantes.complete_messaging_outbox_dead(uuid, integer, text) to authenticated;
