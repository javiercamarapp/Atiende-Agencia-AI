...
# que scripts/verify-hoteles-whatsapp-retry-cap/run.sh (ver ese archivo para el
# porqué de cada paso). Se agrega junto con
# packages/domain-restaurantes/migrations/020_whatsapp_retry_cap.sql para dejar, en
# el repo, la prueba reproducible de que `restaurantes.claim_whatsapp_message` ahora
# topa los reintentos de un mensaje 'failed' contra RLS/GRANT/`auth.uid()` reales --
# el repositorio en memoria de domain-restaurantes nunca aplica ninguno de los tres,
...
#
# Uso:  scripts/verify-restaurantes-whatsapp-retry-cap/run.sh
set -euo pipefail

for bin in initdb pg_ctl psql; do
  if ! command -v "$bin" >/dev/null 2>&1; then
    echo "verify-restaurantes-whatsapp-retry-cap: falta '$bin' en PATH — instala Postgres localmente para correr esta verificación (opcional, no bloquea npm test)." >&2
    exit 1
  fi
done
...
echo "==> aplicando TODAS las migraciones reales de supabase/migrations/ en orden (incluye 020_whatsapp_retry_cap.sql)"
...
echo "==> corriendo fixtures + escenarios del tope de reintentos (ver assertions.sql)"
echo ""
"${PSQL_DB[@]}" -f "$HERE/assertions.sql"

echo ""
echo "==> listo — revisa arriba: los escenarios marcados \"should_fail\"/\"deberia_ser_0\"/\"deberia_ser_false\" deben terminar en ERROR o 0/false (correcto), el resto debe devolver filas/RETURNING reales."
