...
# EXACTO que scripts/verify-superadmin-resumen/run.sh. Se agrega junto con
# `packages/db/migrations/0016_superadmin_acciones.sql` (acciones sugeridas
# con confirmación + dos automatizaciones seguras) para dejar, en el repo,
# la prueba reproducible de que: las automatizaciones (`*_for_system`)
# rechazan a una sesión real; la máquina de estados del intent
# (crear/confirmar/cancelar) ata `creado_por` a `auth.uid()` real contra
# GRANT/RLS reales; dos confirmaciones concurrentes del mismo intent
# ejecutan una sola vez; un intent vencido/ajeno/ya usado no se puede
# confirmar; y la bitácora (`core.automation_action_log`) registra cada
# acción — el repositorio en memoria (InMemorySuperadminAccionesRepository)
# nunca aplica ninguno de esos, así que nunca podría detectar este hueco por
# sí solo.
#
...
#
# Uso:  scripts/verify-superadmin-acciones/run.sh
set -euo pipefail

for bin in initdb pg_ctl psql; do
  if ! command -v "$bin" >/dev/null 2>&1; then
    echo "verify-superadmin-acciones: falta '$bin' en PATH — instala Postgres localmente para correr esta verificación (opcional, no bloquea npm test)." >&2
    exit 1
  fi
done
