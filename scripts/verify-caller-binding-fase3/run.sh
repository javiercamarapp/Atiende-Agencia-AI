...
# EXACTO que scripts/verify-superadmin-caller-binding/run.sh (Fase 1) y
# scripts/verify-caller-binding-fase2/run.sh (Fase 2) de esta misma
# investigación. Se agrega junto con `packages/db/migrations/0016_caller_
# binding_fase3.sql` + las 4 migraciones de vertical hermanas (Fase 3: cierra
# los hallazgos que la Fase 2 dejó documentados como "fuera de alcance" — ver
# `scripts/verify-caller-binding-fase2/README.md`) para dejar, en el repo, la
...
# algo.
#
# Uso:  scripts/verify-caller-binding-fase3/run.sh
set -euo pipefail

for bin in initdb pg_ctl psql; do
  if ! command -v "$bin" >/dev/null 2>&1; then
    echo "verify-caller-binding-fase3: falta '$bin' en PATH — instala Postgres localmente para correr esta verificación (opcional, no bloquea npm test)." >&2
    exit 1
  fi
done
