...
# EXACTO que scripts/verify-rentas-break-glass/run.sh (ver ese archivo para el
# porqué de cada paso). Se agrega junto con
# packages/db/migrations/0020_superadmin_impersonacion.sql (Bloque C:
# impersonación de superadmin con bitácora) para dejar, en el repo, la prueba
# reproducible de que el ciclo de vida completo (iniciar/terminar/leer,
# caller-binding, motivo obligatorio, "nunca impersonar a otro superadmin",
# duración/expiración verificada en SQL, bitácora inmutable) funciona contra
# RLS/GRANT/auth.uid() reales -- el repositorio en memoria de
# packages/db/src/impersonation-repository.ts nunca aplica ninguno de los
# tres, así que nunca podría detectar un hueco de este tipo.
#
...
#
# Uso:  scripts/verify-superadmin-impersonacion/run.sh
set -euo pipefail

for bin in initdb pg_ctl psql; do
  if ! command -v "$bin" >/dev/null 2>&1; then
    echo "verify-superadmin-impersonacion: falta '$bin' en PATH — instala Postgres localmente para correr esta verificación (opcional, no bloquea npm test)." >&2
    exit 1
  fi
done
...
echo "==> aplicando TODAS las migraciones reales de supabase/migrations/ en orden (incluye 0020_superadmin_impersonacion.sql)"
