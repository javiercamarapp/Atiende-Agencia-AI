...
# packages/domain-citas/migrations/023_citas_audit_log.sql para dejar, en el repo,
# la prueba reproducible de que citas.audit_log/citas.record_audit_log funcionan
# contra RLS/GRANT/auth.uid() reales -- el repositorio en memoria de domain-citas
...
#
# Uso:  scripts/verify-citas-audit-log/run.sh
set -euo pipefail

for bin in initdb pg_ctl psql; do
  if ! command -v "$bin" >/dev/null 2>&1; then
    echo "verify-citas-audit-log: falta '$bin' en PATH — instala Postgres localmente para correr esta verificación (opcional, no bloquea npm test)." >&2
    exit 1
  fi
done
...
echo "==> aplicando TODAS las migraciones reales de supabase/migrations/ en orden (incluye 023_citas_audit_log.sql)"
