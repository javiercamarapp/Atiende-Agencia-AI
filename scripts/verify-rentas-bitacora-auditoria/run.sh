...
# cada paso). Se agrega junto con
# packages/domain-rentas/migrations/021_rentas_audit_log.sql para dejar, en el repo,
# la prueba reproducible de que rentas.audit_log/rentas.record_audit_log funcionan
# contra RLS/GRANT/auth.uid() reales -- el repositorio en memoria de domain-rentas
# nunca aplica ninguno de los tres, así que nunca podría detectar un hueco de este
# tipo.
#
...
#
# Uso:  scripts/verify-rentas-bitacora-auditoria/run.sh
set -euo pipefail

for bin in initdb pg_ctl psql; do
  if ! command -v "$bin" >/dev/null 2>&1; then
    echo "verify-rentas-bitacora-auditoria: falta '$bin' en PATH — instala Postgres localmente para correr esta verificación (opcional, no bloquea npm test)." >&2
    exit 1
  fi
done
...
echo "==> aplicando TODAS las migraciones reales de supabase/migrations/ en orden (incluye 021_rentas_audit_log.sql)"
