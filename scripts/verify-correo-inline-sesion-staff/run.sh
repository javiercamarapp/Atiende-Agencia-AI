...
# hallazgo confirmado de la auditoría a2 (CRÍTICO): "el drenado inline de correo
# aborta la transacción de negocio en sesión de staff" (ver assertions.sql para
# el mecanismo completo, ANTES/DESPUÉS del hotfix de SAVEPOINT).
#
...
# algo.
#
# Uso:  scripts/verify-correo-inline-sesion-staff/run.sh
set -euo pipefail

for bin in initdb pg_ctl psql; do
  if ! command -v "$bin" >/dev/null 2>&1; then
    echo "verify-correo-inline-sesion-staff: falta '$bin' en PATH — instala Postgres localmente para correr esta verificación (opcional, no bloquea npm test)." >&2
    exit 1
  fi
done
