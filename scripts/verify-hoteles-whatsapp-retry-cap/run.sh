...
# de cada paso). Se agrega junto con
# packages/domain-hoteles/migrations/027_whatsapp_retry_cap.sql para dejar, en el
# repo, la prueba reproducible de que `hoteles.claim_whatsapp_message` ahora topa
# los reintentos de un mensaje 'failed' contra RLS/GRANT/`auth.uid()` reales -- el
# repositorio en memoria de domain-hoteles nunca aplica ninguno de los tres, así
# que nunca podría detectar un hueco de este tipo.
#
...
#
# Uso:  scripts/verify-hoteles-whatsapp-retry-cap/run.sh
set -euo pipefail

for bin in initdb pg_ctl psql; do
  if ! command -v "$bin" >/dev/null 2>&1; then
    echo "verify-hoteles-whatsapp-retry-cap: falta '$bin' en PATH — instala Postgres localmente para correr esta verificación (opcional, no bloquea npm test)." >&2
    exit 1
  fi
done
...
echo "==> aplicando TODAS las migraciones reales de supabase/migrations/ en orden (incluye 027_whatsapp_retry_cap.sql)"
