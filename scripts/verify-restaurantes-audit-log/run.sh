...
# que scripts/verify-rentas-bitacora-auditoria/run.sh (ver ese archivo para el
# porqué de cada paso). Se agrega junto con
# packages/domain-restaurantes/migrations/019_restaurantes_audit_log.sql para dejar,
# en el repo, la prueba reproducible de que restaurantes.audit_log/
# restaurantes.record_audit_log funcionan contra RLS/GRANT/auth.uid() reales -- el
...
#
# Uso:  scripts/verify-restaurantes-audit-log/run.sh
set -euo pipefail

for bin in initdb pg_ctl psql; do
  if ! command -v "$bin" >/dev/null 2>&1; then
    echo "verify-restaurantes-audit-log: falta '$bin' en PATH — instala Postgres localmente para correr esta verificación (opcional, no bloquea npm test)." >&2
    exit 1
  fi
done
...
echo "==> aplicando TODAS las migraciones reales de supabase/migrations/ en orden (incluye 019_restaurantes_audit_log.sql)"
