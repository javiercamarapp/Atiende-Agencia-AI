...
# EXACTO que scripts/verify-caller-binding-fase2/run.sh (y
# scripts/verify-superadmin-caller-binding/run.sh, su Fase 1). Se agrega junto
# con `packages/db/migrations/0013_superadmin_facturacion.sql` (pantalla
# /superadmin/facturacion — lectura agregada del billing SaaS por
# organización + diagnóstico de webhook) para dejar, en el repo, la prueba
# reproducible de que las 3 funciones nuevas atan `p_caller_id` a
# `auth.uid()` real contra GRANT/RLS reales — el repositorio en memoria
# (InMemoryCoreRepository) nunca aplica ninguno de los dos, así que nunca
# podría detectar este hueco por sí solo.
#
...
#
# Uso:  scripts/verify-superadmin-facturacion/run.sh
set -euo pipefail

for bin in initdb pg_ctl psql; do
  if ! command -v "$bin" >/dev/null 2>&1; then
    echo "verify-superadmin-facturacion: falta '$bin' en PATH — instala Postgres localmente para correr esta verificación (opcional, no bloquea npm test)." >&2
    exit 1
  fi
done
