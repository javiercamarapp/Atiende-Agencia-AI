...
# que scripts/verify-citas-audit-log/run.sh (ver ese archivo para el porqué de cada
# paso). Se agrega junto con
# packages/domain-hoteles/migrations/029_rate_recommendation_engine.sql para dejar,
# en el repo, la prueba reproducible de que
# hoteles.rate_recommendation/pricing_rule/local_event/competitor_rate y
# hoteles.system_apply_rate_recommendation funcionan contra RLS/GRANT/auth.uid()
# reales -- el repositorio en memoria de domain-hoteles nunca aplica ninguno de los
...
#
# Uso:  scripts/verify-hoteles-motor-tarifas/run.sh
set -euo pipefail

for bin in initdb pg_ctl psql; do
  if ! command -v "$bin" >/dev/null 2>&1; then
    echo "verify-hoteles-motor-tarifas: falta '$bin' en PATH — instala Postgres localmente para correr esta verificación (opcional, no bloquea npm test)." >&2
    exit 1
  fi
done
...
echo "==> aplicando TODAS las migraciones reales de supabase/migrations/ en orden (incluye 029_rate_recommendation_engine.sql)"
