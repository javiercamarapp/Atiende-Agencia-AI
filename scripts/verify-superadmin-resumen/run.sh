...
# EXACTO que scripts/verify-superadmin-salud/run.sh. Se agrega junto con
# `packages/db/migrations/0015_superadmin_resumen_diario.sql` (resumen diario
# automático — 15 funciones nuevas + `core.daily_ops_summary`) para dejar, en
# el repo, la prueba reproducible de que las funciones de SOLO-SISTEMA
# (`*_for_system`) rechazan a una sesión real (`auth.uid()` no nulo), que las
# funciones `*_for_superadmin` atan `p_caller_id` a `auth.uid()` real contra
# GRANT/RLS reales, y que el UPSERT/marcado de correo son idempotentes — el
# repositorio en memoria (InMemoryResumenDiarioRepository) nunca aplica
# ninguno de los dos, así que nunca podría detectar este hueco por sí solo.
#
...
#
# Uso:  scripts/verify-superadmin-resumen/run.sh
set -euo pipefail

for bin in initdb pg_ctl psql; do
  if ! command -v "$bin" >/dev/null 2>&1; then
    echo "verify-superadmin-resumen: falta '$bin' en PATH — instala Postgres localmente para correr esta verificación (opcional, no bloquea npm test)." >&2
    exit 1
  fi
done
