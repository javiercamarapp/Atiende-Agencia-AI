...
# cada paso). Se agrega para ejercitar contra Postgres real el ÚNICO hueco real que
# la auditoría del 18-sep señaló para restaurantes: los 443 tests de
# domain-restaurantes corren SIEMPRE contra el repositorio en memoria, nunca contra
# el repositorio Postgres real (postgres-repository.ts) — RPC/RLS/GRANT reales
# jamás se ejercitan.
#
...
#
# Uso:  scripts/verify-restaurantes-sql/run.sh
set -euo pipefail

for bin in initdb pg_ctl psql; do
  if ! command -v "$bin" >/dev/null 2>&1; then
    echo "verify-restaurantes-sql: falta '$bin' en PATH — instala Postgres localmente para correr esta verificación (opcional, no bloquea npm test)." >&2
    exit 1
  fi
done
...
echo "==> corriendo fixtures + escenarios de restaurantes (ver assertions.sql)"
echo ""
