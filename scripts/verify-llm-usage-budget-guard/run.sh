...
# EXACTO que scripts/verify-outbox-grants/run.sh (ver ese archivo para el
# porqué de cada paso). Se agrega junto con el guard `auth.uid() is not null`
# de `core.record_llm_usage`/`core.reserve_llm_monthly_budget`/
# `core.settle_llm_monthly_budget` (revisión de PR sobre
# packages/db/migrations/0010_llm_usage_budget_schema.sql) para dejar, en el
# repo, la prueba reproducible de que el fix funciona contra GRANT/`auth.uid()`
# reales — el repositorio en memoria (InMemoryLlmUsageRepository) nunca aplica
# ninguno de los dos, así que nunca podría detectar este hueco.
#
...
# algo.
#
# Uso:  scripts/verify-llm-usage-budget-guard/run.sh
set -euo pipefail

for bin in initdb pg_ctl psql; do
  if ! command -v "$bin" >/dev/null 2>&1; then
    echo "verify-llm-usage-budget-guard: falta '$bin' en PATH — instala Postgres localmente para correr esta verificación (opcional, no bloquea npm test)." >&2
    exit 1
  fi
done
...
echo "==> corriendo fixtures + escenarios de autorización (6 escenarios, ver assertions.sql)"
echo ""
"${PSQL_DB[@]}" -f "$HERE/assertions.sql"

echo ""
echo "==> listo — revisa arriba: los escenarios marcados 'RECHAZADO' deben terminar en ERROR (correcto), el resto debe devolver una fila/tener éxito."
