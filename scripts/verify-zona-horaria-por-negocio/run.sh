#!/usr/bin/env bash
# Verificación manual, opt-in, contra un Postgres LOCAL real — cierra el hueco de
# test real de la migración 012 (packages/domain-despachos/migrations/
# 012_property_config_zona_horaria.sql) y la migración 022
# (packages/domain-restaurantes/migrations/022_zona_horaria_branch_detail.sql):
# ningún test en memoria aplica RLS/GRANT real, así que ninguno puede demostrar que
# la autorización de "quién puede editar la zona horaria de un negocio" realmente
# funciona contra Postgres real -- positivo, negativo (rol insuficiente), cross-
# tenant y anon. Mismo patrón EXACTO que scripts/verify-outbox-grants (leído primero
# como plantilla).
#
...
#
# Uso:  scripts/verify-zona-horaria-por-negocio/run.sh
set -euo pipefail

for bin in initdb pg_ctl psql; do
  if ! command -v "$bin" >/dev/null 2>&1; then
    echo "verify-zona-horaria-por-negocio: falta '$bin' en PATH — instala Postgres localmente para correr esta verificación (opcional, no bloquea npm test)." >&2
    exit 1
  fi
done
...
echo "==> aplicando TODAS las migraciones reales de supabase/migrations/ en orden (incluye 012/022, esquema completo -- no 'a medio migrar': eso ya lo cubren los tests con AbortAwareFakeSession, ver packages/domain-despachos/tests/postgres-repository-property-config-savepoint.spec.ts y packages/domain-restaurantes/tests/zona-horaria-branch-savepoint.spec.ts)"
...
echo ""
echo "==> listo — revisa arriba: los escenarios marcados 'RECHAZADO'/'debe fallar' deben terminar en ERROR (correcto), el resto debe devolver una fila."
