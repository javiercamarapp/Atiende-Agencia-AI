...
# EXACTO que scripts/verify-superadmin-facturacion/run.sh. Se agrega junto con
# packages/db/migrations/0018_billing_webhook_registro.sql (bitácora completa
# de webhooks de billing SaaS para el superadmin) para dejar, en el repo, la
# prueba reproducible de que `core.record_billing_webhook_event`/`core.
# list_billing_webhook_log_for_superadmin` atan su autoridad real (sesión de
# sistema para escribir, `p_caller_id = auth.uid()` real para leer) contra
# GRANT/RLS reales — el repositorio en memoria (InMemoryCoreRepository) nunca
# aplica ninguno de los dos, así que nunca podría detectar este hueco por sí
# solo.
#
...
#
# Uso:  scripts/verify-billing-webhook-registro/run.sh
set -euo pipefail

for bin in initdb pg_ctl psql; do
  if ! command -v "$bin" >/dev/null 2>&1; then
    echo "verify-billing-webhook-registro: falta '$bin' en PATH — instala Postgres localmente para correr esta verificación (opcional, no bloquea npm test)." >&2
    exit 1
  fi
done
...
echo ""
echo "==> listo — revisa arriba: los escenarios marcados 'RECHAZADO'/'SIN FUGA'/'DEBERIA_SER_0' deben terminar en ERROR o en 0/valor-cero (correcto), el resto debe devolver datos reales."
