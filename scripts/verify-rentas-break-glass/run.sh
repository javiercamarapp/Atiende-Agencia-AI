...
# EXACTO que scripts/verify-superadmin-caller-binding/run.sh (ver ese archivo
# para el porqué de cada paso). Se agrega junto con
# packages/domain-rentas/migrations/018_break_glass_wiring.sql para dejar, en
# el repo, la prueba reproducible de que (1) el criterio débil de
# `rentas.is_platform_superadmin` quedó corregido (delega en
# `core.platform_superadmin`) y (2) el mecanismo completo de "romper cristal"
# (abrir/listar/cerrar ventana + leer datos de tenant SOLO mientras esté
# vigente + bitácora inmutable) funciona contra RLS/GRANT/auth.uid() reales --
# el repositorio en memoria de domain-rentas nunca aplica ninguno de los tres,
...
# algo.
#
# Uso:  scripts/verify-rentas-break-glass/run.sh
set -euo pipefail

for bin in initdb pg_ctl psql; do
  if ! command -v "$bin" >/dev/null 2>&1; then
    echo "verify-rentas-break-glass: falta '$bin' en PATH — instala Postgres localmente para correr esta verificación (opcional, no bloquea npm test)." >&2
    exit 1
  fi
done
...
echo "==> aplicando TODAS las migraciones reales de supabase/migrations/ en orden (incluye 018_break_glass_wiring.sql)"
