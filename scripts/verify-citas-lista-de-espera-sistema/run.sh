#!/usr/bin/env bash
# Verificación contra Postgres LOCAL real de f2-citas-lista-de-espera, hallazgos
# (A)/(C): `citas.appointment_waitlist` no tenía ninguna salida de RLS para la
# sesión de SISTEMA (migración 020_appointment_waitlist_sistema_lectura.sql).
# Ver assertions.sql para el flujo completo (se libera un horario -> se lee la
# lista de espera en sesión de sistema -> queda el aviso encolado), los
# controles negativos/cross-tenant/anon, y el escenario de "esquema de
# producción a medias" (migración 020 no aplicada).
#
...
# real.
#
# Uso:  scripts/verify-citas-lista-de-espera-sistema/run.sh
set -euo pipefail

for bin in initdb pg_ctl psql; do
  if ! command -v "$bin" >/dev/null 2>&1; then
    echo "verify-citas-lista-de-espera-sistema: falta '$bin' en PATH — instala Postgres localmente para correr esta verificación (opcional, no bloquea npm test)." >&2
    exit 1
  fi
done
...
echo ""
echo "==> listo — revisa arriba: los *_deberia_ser_0/1 y los 'as should_fail' son la prueba real del gap de RLS, del fix (función de sistema), de los controles negativos/cross-tenant/anon, y del flujo completo de punta a punta."
