...
| REQ-TEN-006 | **Desactualizado — ver nota.** Al escribir esta fila solo 5 verticales (`hoteles`,`restaurantes`,`rentas`,`licitaciones`,`citas`) eran válidas y `despachos` se rechazaba. `despachos` se incorporó como sexta vertical real en su propia Fase 1 (`packages/domain-despachos/`) — hoy `isVertical`/`VERTICALS` aceptan las 6, y `packages/core-tenancy/tests/types.spec.ts` prueba exactamente esas 6, no el rechazo de `despachos`. Fila conservada íntegra (criterio de esta matriz: no reescribir filas históricas) — ver `docs/ACEPTACION.md` REQ-TEN-006 para el estado vigente. | `packages/core-tenancy/src/types.ts` (`isVertical`) | `packages/core-tenancy/tests/types.spec.ts` | hecho (histórico — vertical rechazada cambió, ver nota) |
