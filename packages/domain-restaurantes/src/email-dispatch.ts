...
// SDK), port 1:1 de @atiende/domain-citas::email-dispatch.ts sobre
// `RestaurantesRepository` en vez de `CitasRepository`. GAP QUE CIERRA
// (hallazgo de auditoría, severidad MEDIA): `restaurantes.messaging_outbox`
// (migrations/007) ya soportaba `channel='email'` desde el día uno, pero
// ningún código real lo reclamaba/completaba — todo el dominio solo encolaba
// `channel: 'whatsapp'` (ver order-notifications.ts).
//
...
export async function sendEmailOutboxJob(fetchImpl: typeof fetch, job: EmailOutboxJobRow, config: ResendConfig): Promise<void> {
  ...
  const response = await fetchImpl("https://api.resend.com/emails", {
    ...
    headers: {
      ...
      // único y estable por fila, mismo criterio que domain-citas.
 ...
 * `POST /internal/restaurantes/email-dispatch` (ver apps/api): reclama hasta
 * `batchSize` jobs `channel='email'` pendientes/fallidos con `attempts < 5`
 * (`restaurantes.claim_email_outbox_batch`), los envía uno por uno, y marca cada
