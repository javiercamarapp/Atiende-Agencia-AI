...
// `PostgresRestaurantesRepository.registrarAuditoria` y `.listAuditoria`. Mismo
// patrón EXACTO que `@atiende/domain-rentas/tests/audit-log-savepoint.spec.ts`
// (y que domain-restaurantes/tests/upsert-customer-savepoint.spec.ts, ya en este
// mismo paquete): un doble de prueba de `TenantDbSession` que SÍ modela el
// comportamiento real de Postgres -- una vez que una consulta dentro de una
// transacción lanza un error, CUALQUIER consulta posterior sin haber pasado por
// `ROLLBACK TO SAVEPOINT` falla con 25P02 "current transaction is aborted...".
// Una sesión falsa plana (que solo registra llamadas, sin modelar el aborto) NO
// puede reproducir el bug real que este fix corrige.
//
// Escenario real que esto reproduce (escritura): `restaurantes.record_audit_log`
// no existe todavía en la base (SQLSTATE 42883, base sin migrar -- ver AGENTS.md
// de esta fase). La acción de negocio (p.ej. `UPDATE restaurantes.products`) YA
...
// `restaurantes.audit_log` todavía no existe (SQLSTATE 42P01, base sin migrar)
// cuando el staff abre la pantalla de Auditoría.
...
function pgUndefinedFunction(): Error & { code: string } {
  const err = new Error("function restaurantes.record_audit_log(uuid, text, text, uuid, text, text, text) does not exist") as Error & { code: string };
  err.code = "42883";
  return err;
}

function pgUndefinedTable(): Error & { code: string } {
  const err = new Error('relation "restaurantes.audit_log" does not exist') as Error & { code: string };
  err.code = "42P01";
  return err;
}
...
class AbortAwareFakeSession implements TenantDbSession {
  ...
  async query<T>(sql: string, _params?: unknown[]): Promise<{ rows: T[] }> {
    ...
    if (normalized.startsWith("select restaurantes.record_audit_log")) {
      this.aborted = true;
    ...
    if (normalized.startsWith("select count(*)::text as total from restaurantes.audit_log")) {
      this.aborted = true;
...
describe("PostgresRestaurantesRepository.registrarAuditoria — recuperación de 42883 con SAVEPOINT", () => {
  it("nunca lanza, y la transacción queda recuperada para la query siguiente del request (sin 25P02)", async () => {
    ...
    expect(session.calls).toContain("savepoint sp_restaurantes_audit_log_write");
    expect(session.calls).toContain("rollback to savepoint sp_restaurantes_audit_log_write");
    expect(session.calls).toContain("release savepoint sp_restaurantes_audit_log_write");
  ...
  it("sin el SAVEPOINT, la misma secuencia SÍ deriva en 25P02 (prueba de que el bug sería real)", async () => {
    ...
    await expect(
      (async () => {
        ...
        try {
          await session.query("select restaurantes.record_audit_log($1,$2,$3,$4,$5,$6,$7);", []);
        } catch {
...
describe("PostgresRestaurantesRepository.listAuditoria — recuperación de 42P01 con SAVEPOINT", () => {
  it("tabla sin migrar: devuelve disponible:false (nunca lanza) y deja la transacción recuperada para la query siguiente", async () => {
    ...
    expect(session.calls).toContain("savepoint sp_restaurantes_audit_log_read");
    expect(session.calls).toContain("rollback to savepoint sp_restaurantes_audit_log_read");
    expect(session.calls).toContain("release savepoint sp_restaurantes_audit_log_read");
  ...
  it("sin el SAVEPOINT, la misma secuencia SÍ deriva en 25P02 (prueba de que el bug de lectura sería real)", async () => {
    ...
    await expect(
      (async () => {
        try {
          await session.query("select count(*)::text as total from restaurantes.audit_log where organization_id = $1;", []);
        } catch {
