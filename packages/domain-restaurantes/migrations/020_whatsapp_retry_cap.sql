...
-- cada reintento de Meta vuelve a gastar un turno de LLM". `restaurantes.
-- claim_whatsapp_message` (redefinida por última vez en
-- `013_rpc_anti_duplicado_authenticated_grants.sql`, sin cambios de fondo desde su
-- creación en `004_whatsapp_atomic_append_and_rate_limit.sql`) re-reclama SIEMPRE
-- un mensaje en estado 'failed', sin importar cuántas veces ya se intentó: Meta
-- reintenta un webhook con 5xx/429 durante DÍAS (comportamiento documentado de
-- Cloud API), y `apps/api/src/routes/verticals/restaurantes/whatsapp.ts` responde
-- 500 mientras `outcome.retryable` sea `true` (ver `handleInboundWhatsAppMessage`,
-- `packages/domain-restaurantes/src/whatsapp/inbound.ts`) — cada uno de esos
-- reintentos vuelve a correr `turnHandler.handleInboundMessage` de punta a punta
-- (llamada real al LLM, gasto real), sin que el cliente reciba jamás una respuesta
-- si la causa del fallo es persistente (p. ej. el catch genérico que introdujo el
-- PR #178).
--
-- Arreglo (mismo diseño que domain-hoteles/migrations/027_whatsapp_retry_cap.sql):
--   1. `whatsapp_inbound_events.attempts` YA existe (desde 001) y YA se incrementa
...
--      ver `011_email_outbox_dispatch.sql`).
...
-- `(uuid, text, text) returns boolean` que `PostgresRestaurantesRepository.
...
-- `scripts/verify-restaurantes-whatsapp-retry-cap/assertions.sql` escenario del
-- "esquema a medio migrar" para la prueba reproducible de ambos comportamientos
-- contra Postgres real.
alter table restaurantes.whatsapp_inbound_events drop constraint whatsapp_inbound_events_status_check;
alter table restaurantes.whatsapp_inbound_events add constraint whatsapp_inbound_events_status_check
...
begin
  ...
  -- `p_organization_id`/`p_message_id` de este caller — nunca toca la fila de otra
  -- organización (imposible de todos modos: `message_id` es la llave primaria y un
  -- mismatch de organization_id ya lo excluyó de la rama de arriba).
