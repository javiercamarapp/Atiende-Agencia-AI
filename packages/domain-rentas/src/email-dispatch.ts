...
// mismo patrón que `domain-citas::email-dispatch.ts` (Fase 6 §3), sobre
// `RentasRepository` en vez de `CitasRepository`.
//
...
// `rentas.claim_email_outbox_batch`, que solo reclama `attempts < 5`) o 'dead' tras
// agotar intentos -- un correo JAMÁS se marca 'sent' sin que Resend en verdad lo
// haya aceptado.
//
...
import type { EmailOutboxJobRow } from "./types.ts";
...
export async function sendEmailOutboxJob(fetchImpl: typeof fetch, job: EmailOutboxJobRow, config: ResendConfig): Promise<void> {
  ...
  const response = await fetchImpl("https://api.resend.com/emails", {
    ...
    headers: {
      ...
      // proveedor sí lo aceptó pero el worker murió antes de marcar 'sent'" para que
      // un reintento del outbox nunca duplique el correo. El id del job ya es único
      // y estable por fila.
      "Idempotency-Key": `rentas-outbox/${job.id}`,
    },
...
/**
 * Reconciliación por lote -- el cuerpo real de `POST /internal/rentas/email-dispatch`
 * (ver apps/api): reclama hasta `batchSize` jobs `channel='email'` pendientes/
 * fallidos con `attempts < 5` (`rentas.claim_email_outbox_batch`), los envía uno por
 * uno, y marca cada uno 'sent' (éxito real de Resend), 'failed' (reintentable, si no
 * agotó los intentos) o 'dead' (agotó los 5 intentos -- un humano debe revisarlo). Un
 * job con datos raros nunca tumba el lote completo de los demás.
 */
export async function dispatchPendingEmailJobs(repo: RentasRepository, config: ResendConfig, opts: { readonly fetchImpl?: typeof fetch; readonly batchSize?: number } = {}): Promise<EmailDispatchSummary> {
