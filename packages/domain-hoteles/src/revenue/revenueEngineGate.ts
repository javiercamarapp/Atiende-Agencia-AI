...
// días, exigir un backtest walk-forward (`walkForwardBacktest.ts`) que demuestre
// mejora vs. baseline antes de poder habilitar "autopilot", pasar entretanto a
// "propone y ejecuta" (cada cambio de tarifa requiere aprobación humana) con un
// límite de variación de ±10-15% hasta que se habilite el autopilot pleno.
//
// Port literal de hoteles/packages/domain-hotel/src/revenue/revenueEngineGate.ts
// (verificado regla por regla contra el original), con UNA diferencia deliberada:
// el original condiciona propone->autopilot también a una "aprobación del fundador"
// (REQ-GOB-012, `founder_reserved_category`) que vive en una tabla de decisiones
// reservadas que este repo NO ha portado todavía (no existe ningún equivalente de
// `founder_reserved_category`/`require_founder_decision_approval` en fusion, y
// `HOTEL_ROLES` de este paquete no distingue "founder" de "owner" — ver roles.ts).
// En vez de fingir esa integración, este port usa el rol real más alto que SÍ existe
// en fusion (`owner`) para la misma exigencia de gobierno: `ownerApprovalGranted` /
// `owner_approved_autopilot_at` en la migración — ver comentario de
// `PromotionContext.ownerApprovalGranted` abajo y REQ-GOB-012 en el resumen de la
// fase para el detalle de qué queda pendiente si el hotel algún día necesita separar
// "owner" de "founder" de verdad.
//
// Espejo de aplicación de la máquina de estados REAL que exige la base de datos
// (`packages/domain-hoteles/migrations/011_revenue_engine_gate.sql`), mismo criterio
// que `reservationStateMachine.ts` frente a `005_reservas_estado.sql`: la autoridad
// final es el trigger de Postgres (nadie puede saltarse esta regla escribiendo SQL a
// mano fuera de la aplicación, y ni siquiera `owner`/`gm` -- que sí tienen permiso de
// RLS para escribir la fila -- pueden pasar a "autopilot" sin que el trigger verifique
// los 90 días, el backtest y la aprobación registrada); este módulo permite
// validar/explicar una transición ANTES de un round-trip a la base y sirve de
// referencia única para cualquier UI/CLI que quiera mostrar por qué una promoción
// está bloqueada.
...
/** Democión = retroceder a un gate más conservador — el "freno de emergencia" (un
 *  gerente que baja el motor a shadow porque sospecha algo no necesita pedirle
 *  permiso a nadie). SIEMPRE permitida, sin ninguna de las condiciones de abajo. */
...
export interface PromotionContext {
  ...
  /** REQ-REV-003 (P0/GOB): el paso de revenue de "propone" a "autopilot" pleno exige
   *  una aprobación explícita registrada del rol `owner` (el más alto de
   *  `HOTEL_ROLES` — ver comentario de cabecera sobre por qué este port usa `owner`
   *  en vez de un "founder" que fusion no modela todavía). Este flag representa si esa
   *  aprobación YA existe y está vigente; la base de datos
   *  (`owner_approved_autopilot_at` + su guarda de rol en la migración) es la
   *  autoridad real, este módulo solo puede reflejar lo que el llamador ya verificó. */
  readonly ownerApprovalGranted?: boolean;
}
 ...
 * mismas reglas — ver 011_revenue_engine_gate.sql).
 */
export function evaluateGateTransition(
  ...
  if (from === "propone" /* && to === "autopilot" */) {
    ...
    if (!ctx.ownerApprovalGranted) {
      reasons.push(
        'aprobacion_owner_requerida: REQ-REV-003 (P0/GOB) exige una aprobación registrada y vigente del rol "owner" antes de habilitar autopilot pleno',
      );
    }
  }
...
export interface RevenueProposalCheck {
   ...
   *  ya esté aprobado: en "propone" sigue faltando el paso real de aprobación humana
   *  del cambio individual (fuera de este módulo, vive en la ruta HTTP/panel que
   *  registra esa aprobación). */
  readonly allowed: boolean;
  readonly reasons: readonly string[];
}
 ...
 * el gate vigente del motor. No sustituye ninguna cola de aprobaciones humana del
 * cambio individual — solo aplica las dos reglas de REQ-REV-003 que son propias del
 * motor de revenue: (a) en "shadow" nunca se ejecuta nada, solo se registra; (b) en
 * "propone" el cambio debe caer dentro del límite de variación vigente y de todos
 * modos requiere aprobación humana antes de ejecutarse.
 */
