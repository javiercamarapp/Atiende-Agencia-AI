// Fase 9 hoteles (REQ-REV-005, P1/F, fuentes BP-003/H02-002): "el agente de revenue
// debe poder explicar en lenguaje natural, en español, por qué recomienda un precio
// (pick-up, compset, evento, tipo de cambio) al consultarlo el dueño."
//
// Port literal de
// hoteles/packages/domain-hotel/src/revenue/priceRecommendationExplainer.ts
// (verificado regla por regla contra el original: ninguna regla de negocio se
// cambió, solo las referencias de paquete/archivo de los comentarios).
//
// Contexto de dominio: cuando el motor de revenue (`revenueEngineGate.ts`) propone
// una tarifa distinta de la vigente, el dueño puede preguntar "¿por qué este
// precio?" y la respuesta debe ser trazable a los factores concretos que la
// produjeron -- nunca una paráfrasis vaga tipo "el mercado lo pide".
//
// Este módulo es dominio puro determinista -- NUNCA usa un LLM para redactar la
// explicación. Mismo principio de honestidad que el resto de este repo aplica al
// CÁLCULO de un precio final (ningún módulo de dominio -- `quote.ts`,
// `folioEngine.ts` -- delega esa cuenta a un modelo de lenguaje) pero un paso más
// allá: tampoco delega la EXPLICACIÓN del precio a un LLM, para que la razón que el
// dueño lee sea siempre trazable a un número concreto que este módulo puede probar
// con datos fijos, nunca a una paráfrasis que un modelo podría inventar, omitir o
// exagerar. Si en el futuro un agente conversacional envuelve esta salida con un tono
// más natural, ese agente NARRA estos hechos ya calculados -- nunca los inventa.
//
// No consulta ningún feed externo: pick-up, compset, evento y tipo de cambio ya
// vienen calculados/obtenidos por quien llama -- mismo patrón de separación de capas
// que `parity-guard.ts` y `compsetGuard.ts`. Este módulo solo ordena los factores por
// relevancia (mayor magnitud primero, para que el dueño lea la razón principal
...
export interface PickupFactor {
  ...
  /** Reservas en libro vs. lo esperado por el pronóstico histórico, en % -- positivo
   *  = pick-up por encima de lo esperado (empuja el precio al alza), negativo = por
   *  debajo (empuja a la baja). */
...
export interface CompsetFactor {
   ...
   *  redacta el resultado ya validado por `compsetGuard.ts`), en % -- positivo =
   *  tarifa propia por encima de la mediana del compset. */
