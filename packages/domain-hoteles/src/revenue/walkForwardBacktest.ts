// Fase 9 hoteles (REQ-REV-003, P0/GOB) — backtesting walk-forward SIN fuga de
// información hacia el futuro: cada ventana de prueba solo puede compararse contra
// datos de ANTES de sí misma. Port literal de
// hoteles/packages/domain-hotel/src/revenue/walkForwardBacktest.ts (verificado
// cifra por cifra contra el original: ninguna regla de negocio se cambió, solo las
// referencias de paquete/archivo de los comentarios) — espejo de la regla de negocio
// para que apps/api y el futuro panel de revenue puedan explicar/probar el cálculo
// sin depender de Postgres, mismo criterio que `revenueEngineGate.ts`/
// `checador/attendance.ts`.
//
// Walk-forward (a diferencia de un split train/test único) evalúa el motor en varias
// ventanas SECUENCIALES, cada una entrenada solo con datos anteriores a su propia
// ventana de prueba — nunca con datos futuros, requisito mínimo para que "mejora vs.
// baseline" sea creíble.
//
// Este módulo DELIBERADAMENTE no inventa un modelo de elasticidad de demanda propio:
// no hay forma honesta de saber qué habría ocupado un hotel a un precio que nunca se
...
// fabricada (mismo criterio de honestidad que el resto de `revenue/` y que
// `folioEngine.ts`: nunca inventar un número que no se pueda trazar). Por eso quien
// llama a `evaluateWalkForwardBacktest` debe: (a) declarar su `counterfactualMethod`
// explícitamente, y (b) aportar, YA CALCULADO por fuera de este módulo (con ese
// método), el ingreso de cada ventana tanto para el motor como para el baseline. Este
// módulo se limita a dos responsabilidades deterministas y auditables: construir las
// ventanas walk-forward sin fuga de datos (`buildWalkForwardWindows`) y adjudicar
// pass/fail sobre los resultados ya calculados (`evaluateWalkForwardBacktest`) con un
// criterio fijo y explicado.
//
// Alcance de este port (Fase 9, ver también revenueEngineGate.ts): este módulo
// implementa el cálculo puro completo del backtest. Lo que NO se construye en esta
// fase es el pipeline que alimenta `WindowEvaluation.engineRevenue`/`baselineRevenue`
// con datos reales de producción (eso exige tener el propio motor de recomendación de
// tarifas corriendo en shadow por 90 días primero, ver `revenueEngineGate.ts`) — ese
// pipeline de ingesta queda documentado como pendiente, no fingido como completo.
