// Fase 9 hoteles (REQ-REV-007, P1/GOB, fuentes BP-047/BP-156): "el sistema debe
// respetar la paridad contractual vigente con OTAs mediante un 'parity guard'
// configurable por hotel (bloquea/alerta si una tarifa propuesta rompe la paridad
// pactada)".
//
// Port literal de hoteles/packages/domain-hotel/src/revenue/parity-guard.ts
...
//
// Este módulo es puro dominio/decisión: NO consulta la tarifa vigente en cada OTA
// (eso requeriría un conector/channel manager que este repo deliberadamente NO
// construye en esta fase) ni escribe nada — solo decide, dada una tarifa directa
// propuesta y las tarifas de referencia YA obtenidas por quien llama, si esa
// propuesta respeta la paridad pactada, y si por eso debe bloquearse o solo generar
// una alerta, según el modo configurado para el hotel. Mismo patrón de separación de
// capas que `revenueEngineGate.ts`: la decisión determinista vive aquí, la
// orquestación (leer tarifas OTA, persistir la alerta, notificar) vive en `apps/api`.
//
...
export interface ParityChannelConfig {
   ...
   *  cerrado de OTAs (evita acoplar domain-hoteles al registro de conectores PMS). */
...
export function evaluateParityGuard(config: ParityGuardConfig, proposedRate: number): ParityCheckResult {
  ...
  // las violaciones para que quien llama decida notificar/registrar sin impedir el
  // cambio.
