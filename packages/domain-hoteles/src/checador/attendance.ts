// Fase 8 hoteles (REQ-BO-024, P0/GOB, LFT art.132 fr.XXXIV) — cruce puro (sin I/O) de
// lo REALMENTE trabajado (eventos `entrada`/`salida` del checador,
// `hoteles.attendance_log` en packages/domain-hoteles/migrations/010_checador_asistencia.sql)
// contra el horario programado (`hoteles.staff_schedule`, misma migración), para
// marcar horas extra NO autorizadas. Port literal del cálculo real de
// hoteles/packages/domain-hotel/src/attendance.ts (verificado cifra por cifra contra
// el original: ninguna regla de negocio se cambió, solo el nombre del paquete de
// origen en los comentarios) — espejo de la regla de negocio para que apps/api y el
// futuro panel de staff puedan explicar/probar el cálculo sin depender de Postgres,
// mismo criterio que `turnos-lft.ts`/`overbooking.ts`/`taxes.ts`.
//
...
//
// Distinto de `housekeeping/turnos-lft.ts` (migrations/009): turnos-lft valida la
// PLANTILLA de turnos ANTES de publicarse (el horario PLANEADO); este módulo cruza
// el horario YA publicado (`staff_schedule`) contra lo que el checador registró que
// REALMENTE pasó (`attendance_log`) -- propósito distinto, archivo distinto.
...
export interface AttendanceSchedule {
   ...
   *  en minutos. Nunca negativo -- ver constraint de la migración 010. */
...
export interface AttendanceCrossCheckResult {
   ...
   *  `apps/api/src/routes/verticals/hoteles/asistencia.ts` expone para alertar. */
  alert: boolean;
}
...
// que ya usa este repo para reportes financieros exportables, ver CFDI de hospedaje).
