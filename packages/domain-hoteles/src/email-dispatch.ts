...
// (`attempts += 1` dentro de `hoteles.claim_email_outbox_batch`, ver su
// migración). Con producción sin la key configurada, cada invocación --el cron
// diario Y, desde #166, el drenado post-commit de CADA acción de staff, ver
// `apps/api/.../hoteles/email-dispatch.ts`-- quemaba un intento de CADA correo
// pendiente de CADA organización; 5 invocaciones bastaban para dejar todos los
// correos pendientes de la plataforma en 'dead' para siempre, sin que Resend
// jamás hubiera visto ninguno. Fix: sin `apiKey`, `dispatchPendingEmailJobs`
// devuelve `{ ...summary vacío, notConfigured: true }` de inmediato, SIN llamar
// al claim -- cero jobs reclamados, cero intentos quemados.
