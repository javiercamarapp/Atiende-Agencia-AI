// Fase 11 hoteles (REQ-CRM-002, P1/F) — clasificador de reseñas/encuestas por TEMA +
// SENTIMIENTO, y decisión de la acción reglada correspondiente (ticket de
// mantenimiento, mensaje proactivo, compensación reglada).
//
// Port literal de hoteles/packages/domain-hotel/src/reputacion/clasificador.ts
// (verificado regla por regla contra el original: ninguna regla de negocio se
// cambió, solo las referencias de paquete/archivo de los comentarios). Gap real
// verificado contra el código de main antes de construirse: ningún archivo bajo
// `packages/domain-hoteles` mencionaba reputación/reseñas/CRM, y
// `packages/domain-hoteles/README.md` listaba "reputación" explícitamente fuera de
// Fase 1, sin que ninguna fase posterior la retomara.
//
// Deterministico y PURO (sin I/O, mismo principio que fraude/deteccion.ts): nada
// aquí llama a un LLM ni a un servicio de terceros -- el criterio de REQUISITOS.md
// del origen declara la dependencia de este requisito como "ninguna", y REQ-CRM-001
// (el inbox real de Google/Booking/TripAdvisor) sigue "pendiente-credenciales" en el
// origen -- este módulo clasifica CUALQUIER texto de reseña/encuesta que ya llegó al
// sistema (por el canal que sea, hoy típicamente una encuesta propia capturada por
// el staff, o una reseña pública copiada/pegada manualmente), sin fabricar ni asumir
// esa integración pendiente. Ver `migrations/013_reputacion.sql` para el modelo de
// datos que persiste la clasificación (`hoteles.guest_review`/
// `hoteles.guest_review_action`), pensado para captura manual/webhook, NUNCA para
// una ingesta automática real de esas plataformas (fuera de alcance de esta fase por
// falta de credenciales, ver `packages/domain-hoteles/README.md`).
//
// "Temas locales no entrenados previamente" (parte explícita del criterio de
// aceptación del origen): en vez de requerir una lista cerrada de temas o un modelo
// entrenado, `detectarTemas` primero busca el diccionario base (+ el diccionario
// propio que cada hotel puede configurar, `temasLocalesConfigurados`, para temas ya
// vistos antes) y SIEMPRE intenta además un descubrimiento heurístico: cualquier
// palabra de contenido (no genérica) que aparece cerca de una palabra con carga de
// sentimiento y que ningún tema conocido/configurado ya cubrió se reporta como un
// tema local nuevo (`local:<palabra>`, `esConocido: false`) -- exactamente el caso
// "sargazo" antes de que alguien lo hubiera agregado nunca a ningún diccionario,
// generalizado a cualquier palabra nueva (una plaga local, un olor, una obra de
// construcción vecina).
 ...
 *  que nunca pueda colisionar por accidente con un `KnownReviewTopic` real, y para
 *  que el conteo de menciones repetidas del MISMO tema pueda seguir agrupando por
 *  esta cadena estable aunque el tema nunca se haya visto antes de esta reseña. */
...
/** Temas que corresponden a un defecto FÍSICO/operativo reparable -- solo estos
 *  disparan `ticket_mantenimiento` (un tema como "precio" o "personal" no se arregla
 *  con un ticket de mantenimiento). */
...
export interface TopicMatch {
   ...
   *  base ni en `temasLocalesConfigurados`) -- criterio explícito del origen: "temas
   *  locales no entrenados previamente". */
  esConocido: boolean;
  menciones: number;
 ...
 * (`TOPIC_KEYWORDS`) y cualquier diccionario propio del hotel
 * (`temasLocalesConfigurados`, temas ya vistos antes y ahora "entrenados" para ese
 * hotel), y SIEMPRE, además, un descubrimiento heurístico de temas nunca vistos
 * (criterio del origen: "temas locales no entrenados previamente") -- una palabra de
 * contenido cerca de una palabra con carga de sentimiento que ningún tema ya cubrió.
 */
 ...
 * maneja negación ("no", "nunca"... invierte el signo de la palabra que sigue,
 * ventana de 3 tokens) e intensificadores ("muy", "super"... multiplica la
 * magnitud, ventana de 2 tokens). Si se provee una `calificacion` (1-5 estrellas),
 * se combina en partes iguales con el puntaje léxico -- una reseña de 5 estrellas
 * con texto breve y neutro sigue siendo positiva, y viceversa.
 */
...
/** Tabla REGLADA (no discrecional) -- solo los temas más severos y objetivamente
 *  verificables tienen una compensación preconfigurada; el resto de los temas nunca
 *  dispara `compensacion_reglada`. */
 ...
 * (ticket, mensaje proactivo, compensación reglada). Puede devolver más de una
 * acción (p.ej. un problema de aire acondicionado severo durante la estancia
 * dispara TICKET + mensaje proactivo), o ninguna (reseña neutra/positiva, o
 * negativa sin huésped identificado al que contactar/compensar).
 */
 ...
 *  único pensado para la futura capa de `apps/api` (ver comentario de cabecera del
 *  archivo sobre lo que SÍ y NO cubre esta fase). */
