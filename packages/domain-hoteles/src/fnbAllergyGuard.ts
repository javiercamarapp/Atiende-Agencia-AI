/**
 * Port literal de hoteles/packages/domain-hotel/src/fnbAllergyGuard.ts (REQ-AB-004,
 * P0/GOB): "Cuando el huésped declare alergia/restricción alimentaria en un pedido de
 * F&B, la orden debe marcarse explícitamente y requerir confirmación humana del
 * cocinero antes de que el sistema asegure al huésped que el platillo es seguro; sin
 * confirmación, el sistema no debe afirmarlo." Módulo de dominio PURO: ninguna función
 * de aquí toca I/O — `apps/api/src/routes/verticals/hoteles/pedidosFnb.ts` es quien
 * persiste el estado y decide cuándo invocar la guarda antes de responder al huésped.
 *
 ...
 *      campo estructurado pero SÍ lo escribió — si hay duda razonable se trata como
 *      declarado, nunca al revés.
...
// celiaca, anafilaxia, "no puedo comer X", "sin gluten/lácteos/nueces por salud") — un
// falso positivo aquí solo cuesta pedir una confirmación de más al cocinero; un falso
// negativo deja pasar un pedido con riesgo real sin marcar. Fail-closed. Se aplica
// SOBRE TEXTO SIN ACENTOS (ver `stripDiacritics`) para reconocer por igual
// "alérgico"/"alergico" y "célíaco"/"celiaco".
...
/** Determina si un pedido debe tratarse como "con alergia/restricción declarada" — ya
 *  sea porque el campo estructurado lo dice, o porque alguna nota de texto libre
 *  PARECE declararlo. */
export function resolveAllergyDeclared(input: ResolveAllergyDeclaredInput): ResolveAllergyDeclaredResult {
  ...
  // MITIGACIÓN INTERIM (heredada de hoteles, P0/SEG): una auditoría adversarial
  // encontró que ALLERGY_KEYWORDS_RE es trivialmente bypasseable con lenguaje
  // natural/coloquial/mal escrito ("no tolero los mariscos, me hace mal comerlos",
  // "quedé hospitalizado"). Mientras no exista una red de seguridad categóricamente
  // distinta, CUALQUIER nota de texto libre no vacía que NO calce con el regex se
  // trata igual que si calzara: `allergyDeclared=true`, exige confirmación de cocina
  // antes de asegurar seguridad. Sobre-disparar es aceptable; NO disparar en un
  // pedido con alergia real no lo es.
...
/** Texto listo para mostrar/enviar al huésped según el estado de confirmación — regla
 *  de dominio pura (sin I/O); la capa de mensajería/ruta decide cuándo enviarlo. El
 *  mensaje de "pendiente" NUNCA afirma que el platillo es seguro. */
