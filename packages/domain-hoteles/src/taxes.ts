// Port literal de hoteles/packages/domain-hotel/src/taxes.ts (H4/REQ-REV-001). IVA/ISH
// son PARÁMETROS por property (hoteles.tax_config), nunca una tasa fiscal fija "de
// verdad" en código — este módulo solo aplica lo que le pasan, nunca decide ni asume
// una tasa por su cuenta. Ningún LLM invoca esta función con un monto propio: siempre
// recibe `netAmount` ya calculado desde tarifas reales.
import { roundCurrency } from "./money.ts";

export interface TaxConfig {
  /** 0..1 (ej. 0.16 = 16%). Configurado por property, ver `hoteles.tax_config.iva_rate`. */
  ivaRate: number;
  /** 0..1 (ej. 0.03 = 3%). Configurado por property, ver `hoteles.tax_config.ish_rate`. */
  ishRate: number;
}
