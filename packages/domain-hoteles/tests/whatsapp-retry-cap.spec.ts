...
// (`v_max_attempts` dentro de `hoteles.claim_whatsapp_message`) vive en
// `migrations/027_whatsapp_retry_cap.sql` -- ver
// `scripts/verify-hoteles-whatsapp-retry-cap/assertions.sql` para la prueba
...
// que no reproduce el SQL real de `claim_whatsapp_message`) -- aquí el mock del
// `claim_whatsapp_message` de la sesión SIMULA el tope real (true en los
// intentos 1..MAX, false a partir de MAX+1), exactamente el contrato que la
// migración 027 le da a Postgres real.
...
// packages/domain-hoteles/migrations/027_whatsapp_retry_cap.sql.
const MAX_WHATSAPP_ATTEMPTS = 5;

describe("handleInboundWhatsAppMessage (hoteles) — tope de reintentos: el N-ésimo intento NO vuelve a invocar al LLM", () => {
  it(`reclama los intentos 1..${MAX_WHATSAPP_ATTEMPTS} (turnHandler corre cada vez, y falla cada vez -- mensaje persistentemente problemático) y el intento ${MAX_WHATSAPP_ATTEMPTS + 1} ya NO reclama: turnHandler NUNCA se invoca esa vez, sin lanzar`, async () => {
    ...
    const session = new AbortAwareFakeSession([
      {
        match: /select hoteles\.claim_whatsapp_message/,
        respond: () => {
          claimCallCount += 1;
          // Mismo contrato que la función SQL real tras 027_whatsapp_retry_cap.sql:
          // reclama mientras `attempts <= MAX`, deja de reclamar (y el mensaje queda
          // 'attempts_exhausted') a partir de ahí -- ver
          // scripts/verify-hoteles-whatsapp-retry-cap/assertions.sql para la prueba
          // de ESE contrato contra Postgres real.
      ...
      // `appendWhatsAppUserMessageOnce`/`whatsappAppendTurn` resuelven
      // `organization_id` desde `property_id` primero (ver
      // `PostgresHotelesRepository.resolvePropertyOrganization`).
    ...
    const alwaysFailingTurnHandler: HotelesWhatsAppTurnHandler = {
      async handleInboundMessage() {
        turnHandlerCallCount += 1;
        // El huésped tiene una causa PERSISTENTE (p. ej. el catch genérico del PR
    ...
    const args = { organizationId, propertyId, messageId, phone: "+5219990000000", body: "hola, necesito ayuda", phoneNumberId: "1234567890" };
    ...
    // HTTP -- ver apps/api/src/routes/verticals/hoteles/whatsapp.ts) SIN lanzar y
    // SIN volver a invocar turnHandler -- Meta deja de reintentar, y ya no se gasta
    // ningún turno más del LLM.
    ...
    const claimConversationCalls = session.calls.filter((c) => c.startsWith("select hoteles.claim_whatsapp_conversation")).length;
  ...
  it("un mensaje que YA no se reclama (claimed = false) nunca invoca al turnHandler, sin importar la causa (mismo camino que un mensaje ya 'processed' o ya 'attempts_exhausted')", async () => {
    ...
    const session = new AbortAwareFakeSession([{ match: /select hoteles\.claim_whatsapp_message/, respond: () => [{ claim_whatsapp_message: false }] }]);
    const repo = new PostgresHotelesRepository(session);

    let turnHandlerCallCount = 0;
    const turnHandler: HotelesWhatsAppTurnHandler = {
      async handleInboundMessage() {
        turnHandlerCallCount += 1;
        return { reply: "no debería llegar aquí", fnbOrderId: null };
      },
    };
    ...
    expect(session.calls).toEqual(["select hoteles.claim_whatsapp_message($1, $2, $3) as claim_whatsapp_message;"]);
  });
});
