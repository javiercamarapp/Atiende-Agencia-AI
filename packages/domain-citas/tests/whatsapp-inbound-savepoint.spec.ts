...
// su cuerpo (guardia de crisis, lookup de cliente, turno, `whatsappAppendTurn`,
// `enqueueMessagingOutbox`, `finishWhatsAppMessage`) en la ÚNICA transacción del
// webhook (`ManagedPostgresEngine.withAppSession`), DENTRO del callback que recibe
// `withConversationLock`. Antes de este fix, si CUALQUIERA de esos pasos lanzaba un
// error real de Postgres, la transacción quedaba ABORTADA (25P02) y el `catch`
// exterior reutilizaba esa MISMA sesión ya abortada para
...
// re-corren el turno LLM completo (gasto real) sin que el cliente reciba jamás una
// respuesta. Además, `finishWhatsAppMessage("processed", ...)` corría ANTES de este
// fix DESPUÉS de resolver el guard, sin ninguna protección propia -- se movió
// dentro del bloque protegido por el mismo motivo.
//
// Mismo diseño de test que `@atiende/domain-hoteles`/`@atiende/domain-restaurantes`
// (ver sus `tests/whatsapp-inbound-savepoint.spec.ts`): `AbortAwareFakeSession`
// (reproduce la semántica REAL de una transacción de Postgres) +
// `PostgresCitasRepository` real (nunca `InMemoryCitasRepository`, que no tiene
// transacción real que abortar) + un `WhatsAppTurnHandler` de prueba (sin LLM) +
// `createDefaultConversationGuard({})` (sin credenciales de Upstash ->
// `InMemoryLockStore` real, sin red, suficiente para serializar dentro de este
// proceso de test).
...
function genericPostgresError(): Error & { code: string } {
  ...
  const err = new Error('relation "citas.messaging_outbox" does not exist') as Error & { code: string };
  err.code = "42P01";
  return err;
}
...
describe("handleInboundWhatsAppMessage (citas) — SAVEPOINT alrededor del turno completo, contra Postgres real (AbortAwareFakeSession)", () => {
  it("enqueueMessagingOutbox dispara un error real de Postgres DESPUÉS de que el turno ya corrió: el fallo queda REGISTRADO (finish_whatsapp_message 'failed' corre y no lanza), el handler responde retryable sin lanzar, y la sesión queda utilizable para el resto del request", async () => {
    ...
    const session = new AbortAwareFakeSession([
      { match: /select citas\.claim_whatsapp_message/, respond: () => [{ claim_whatsapp_message: true }] },
      { match: /select citas\.append_whatsapp_user_message_once/, respond: () => [] },
      // Sin `citas.tenant_config` -- la guardia de crisis corta ahí (`triggered: false`).
      { match: /select organization_id, rubro, default_timezone, owner_notification_phone from citas\.tenant_config/, respond: () => [] },
      // Cliente nunca visto -- `lookupCitasCustomer` corta ahí, sin más consultas.
      { match: /select id, organization_id, full_name, phone, email from citas\.customers/, respond: () => [] },
      { match: /select citas\.whatsapp_append_turn/, respond: () => [] },
      // El paso que revienta -- DESPUÉS de que el turno completo ya produjo una
      // respuesta y se guardó en el historial.
      { match: /select citas\.enqueue_messaging_outbox/, respond: () => genericPostgresError() },
      { match: /select citas\.finish_whatsapp_message/, respond: () => [] },
      { match: /select 1/, respond: () => [] },
    ]);
    const repo = new PostgresCitasRepository(session);
    const guard = createDefaultConversationGuard({});

    const outcome = await handleInboundWhatsAppMessage(repo, fixedReplyTurnHandler, guard, {
      organizationId,
      messageId,
      phone: "+5219990000000",
    ...
    const finishCall = session.calls.find((c) => c.startsWith("select citas.finish_whatsapp_message"));
    ...
    const finishIdx = session.calls.findIndex((c) => c.startsWith("select citas.finish_whatsapp_message"));
  ...
  it("control: la MISMA secuencia con un catch simple (sin SAVEPOINT) deja la sesión abortada -- prueba de que el bug era real", async () => {
    const session = new AbortAwareFakeSession([{ match: /select citas\.enqueue_messaging_outbox/, respond: () => genericPostgresError() }]);

    await expect(
      (async () => {
        try {
          await session.query("select citas.enqueue_messaging_outbox($1, $2, $3, $4, $5::jsonb);", []);
        } catch {
          // catch simple -- exactamente el patrón roto que `handleInboundWhatsAppMessage`
          // tenía antes de envolver el cuerpo del callback de `withConversationLock` en
          // `repo.runWithRowSavepoint`.
        }
        // El catch roto intentaría registrar el fallo aquí, sobre la sesión ya abortada.
        return session.query("select citas.finish_whatsapp_message($1, $2, $3, $4, $5);", []);
      })(),
