...
describe("tryTriggerCalendarSync / syncPendingAppointmentsMultiProvider — despacho genérico por plataforma", () => {
  ...
  it("un 401 real de Cal.com (credencial revocada) marca la CUENTA CAL.COM en error de una vez, sin quemar reintentos", async () => {
    const fixture = buildCitasFixture();
    await fixture.repo.connectProviderCalComAccount({ organizationId: fixture.organizationId, providerId: fixture.providerId, calcomEventTypeId: "555", apiKey: "cal_test_revocada" });
    const appointment = await createRealAppointment(fixture);
  ...
  it("un 403 real de CalDAV (contraseña de aplicación revocada) marca la CUENTA CALDAV en error de una vez", async () => {
    const fixture = buildCitasFixture();
    await fixture.repo.connectProviderCalDavAccount({ organizationId: fixture.organizationId, providerId: fixture.providerId, calendarCollectionUrl: "https://caldav.example.com/cal/", username: "x@y.com", password: "revocada" });
    const appointment = await createRealAppointment(fixture);
  ...
  it("un fallo transitorio (no 401/403) reintenta con backoff, nunca marca la cuenta en error", async () => {
    const fixture = buildCitasFixture();
    await fixture.repo.connectProviderCalComAccount({ organizationId: fixture.organizationId, providerId: fixture.providerId, calcomEventTypeId: "555", apiKey: "cal_test" });
    const appointment = await createRealAppointment(fixture);
...
describe("createCalendarSyncPortResolver — de punta a punta contra los simuladores HTTP reales", () => {
  ...
  it("Cal.com conectado: el resolver real construye un RealCalComPort apuntando al simulador y el motor crea el booking real", async () => {
    const fixture = buildCitasFixture();
    calcomSim = new CalComApiSimulator({ apiKey: "cal_test_e2e", eventTypes: [{ id: 555, slug: "consulta", title: "Consulta", lengthInMinutes: 30 }] });
    const { baseUrl } = await calcomSim.start();

    await fixture.repo.connectProviderCalComAccount({ organizationId: fixture.organizationId, providerId: fixture.providerId, calcomEventTypeId: "555", apiKey: "cal_test_e2e", baseUrl });
    const appointment = await createRealAppointment(fixture);
  ...
  it("CalDAV conectado: el resolver real construye un RealCalDavPort apuntando al simulador y el motor hace PUT del VEVENT real", async () => {
    const fixture = buildCitasFixture();
    caldavSim = new CalDavServerSimulator({ username: "x@y.com", password: "app-pw-e2e", calendarPath: "/dav/calendars/x/" });
    const { calendarCollectionUrl } = await caldavSim.start();

    await fixture.repo.connectProviderCalDavAccount({ organizationId: fixture.organizationId, providerId: fixture.providerId, calendarCollectionUrl, username: "x@y.com", password: "app-pw-e2e" });
    const appointment = await createRealAppointment(fixture);
