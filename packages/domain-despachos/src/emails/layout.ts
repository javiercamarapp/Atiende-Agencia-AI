// Marco visual compartido de todos los correos transaccionales de despachos —
// port EXACTO (mismo HTML, misma paleta) de
// packages/domain-citas/src/emails/layout.ts/packages/domain-rentas/src/emails/layout.ts/
// packages/domain-licitaciones/src/emails/layout.ts (leídos primero como
// plantilla): tabla HTML compatible Outlook/Gmail/Apple Mail, wordmark de
// texto "atiende" (NUNCA un logo de imagen embebido — ninguna vertical del
// monorepo lo usa, ver línea del wordmark abajo), tarjeta blanca, preheader
// oculto. Sigue siendo la marca "atiende" (un solo software/frontend para
// todas las verticales) — el hallazgo de auditoría que agrega este archivo es
// que despachos no reutilizaba NADA de esta infraestructura ya real en el
// monorepo, no que hiciera falta una nueva.
//
// escapeHtml se EXPORTA (nunca privada) porque el dato dinámico más sensible
// de este dominio es el nombre del emisor/cliente de una factura o el
// nombre/razón social de la organización, que llegan tal cual los capturó el
// panel — nunca confiables sin escapar.
  ...
  <tr><td align="center" style="padding:44px 16px 36px 16px;">
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="600" style="width:600px;max-width:100%;border-collapse:collapse;">
      ...
      <tr><td align="left" style="padding:26px 6px 0 6px;">
        <p style="margin:0 0 7px 0;font-family:${FUENTE};font-size:11px;line-height:17px;font-weight:600;letter-spacing:0.08em;text-transform:uppercase;color:#5b6b82;">atiende&nbsp;&nbsp;&#183;&nbsp;&nbsp;Contabilidad y cumplimiento fiscal con IA</p>
