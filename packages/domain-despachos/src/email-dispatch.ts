...
// SDK), port EXACTO de `packages/domain-citas/src/email-dispatch.ts`/
// `packages/domain-licitaciones/src/email-dispatch.ts` (leídos primero como
// plantilla) sobre `DespachosRepository` en vez de `CitasRepository`/
...
// `despachos.claim_email_outbox_batch`, que solo reclama `attempts < 5`) o
...
import type { DespachosRepository, EmailOutboxJobRow } from "./repository.ts";
 ...
 * `POST /internal/despachos/email-dispatch` (ver apps/api): reclama hasta
 * `batchSize` jobs `channel='email'` pendientes/fallidos con `attempts < 5`
 * (`despachos.claim_email_outbox_batch`), los envía uno por uno, y marca cada
 * uno 'sent' (éxito real de Resend), 'failed' (reintentable, si no agotó los
 ...
 */
export async function dispatchPendingEmailJobs(repo: DespachosRepository, config: ResendConfig, opts: { readonly fetchImpl?: typeof fetch; readonly batchSize?: number } = {}): Promise<EmailDispatchSummary> {
