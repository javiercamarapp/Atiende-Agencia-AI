...
describe("agregarDiot — contrato de API y edge cases", () => {
  ...
  it("tasa 0.16 exacta mapea a tipoOperacion 03", () => {
    const r = agregarDiot([candidato({ tasaIva: 0.16 })], "DESP010101AB1", "2026-07");
    expect(r.registros[0]!.tipoOperacion).toBe("03");
  });

  it("tasa 0 exacta mapea a tipoOperacion 06", () => {
    const r = agregarDiot([candidato({ tasaIva: 0, ivaTrasladado: 0, ivaAcreditable: 0 })], "DESP010101AB1", "2026-07");
    expect(r.registros[0]!.tipoOperacion).toBe("06");
  });

  it("tasa 0.08 (frontera) mapea a tipoOperacion 85 y su IVA cae en ivaExento", () => {
  ...
  it("tasa exenta/otra (ej. 0.04) también mapea a 85 (Otros)", () => {
    const r = agregarDiot([candidato({ tasaIva: 0.04, ivaTrasladado: 40 })], "DESP010101AB1", "2026-07");
    expect(r.registros[0]!.tipoOperacion).toBe("85");
  });
  ...
  it("ordena los registros por (rfc, tipoOperacion) — comparación de string, determinista", () => {
    const r = agregarDiot(
      [
        candidato({ rfcEmisor: "ZZZ010101AB1", tasaIva: 0.16 }),
        candidato({ rfcEmisor: "AAA010101AB1", tasaIva: 0 }),
        candidato({ rfcEmisor: "AAA010101AB1", tasaIva: 0.16 }),
      ],
      "DESP010101AB1",
      "2026-07",
    );
