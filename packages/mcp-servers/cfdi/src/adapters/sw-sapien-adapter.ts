/**
 * Adaptador real contra SW Sapien/"SW" (PAC secundario de ejemplo) — segundo PAC
 * intercambiable, mitigación de "CFDI de hospedaje mal timbrado". Mismo esqueleto
 * honesto que `FinkokAdapter`. Puerto ~literal de
 * `hoteles/packages/mcp-servers/cfdi/src/adapters/sw-sapien-adapter.ts`.
 *
 * [PENDIENTE DE CREDENCIALES] — requiere `SW_API_TOKEN`, `SW_CSD_CERT_PATH`,
 * `SW_CSD_KEY_PATH`, `SW_CSD_PASSWORD`, `SW_WEBHOOK_SECRET`. Sin ellas, este
 * adaptador SIEMPRE lanza `PortUnavailableError` — la suite de pruebas nunca
 * depende de que estas variables existan (usa `FakeSwSapienAdapter`).
 */
...
const REQUIRED_ENV = ["SW_API_TOKEN", "SW_CSD_CERT_PATH", "SW_CSD_KEY_PATH", "SW_CSD_PASSWORD", "SW_WEBHOOK_SECRET"] as const;
...
export class SwSapienAdapter implements CfdiPort {
  ...
  status(): AdapterStatus {
    ...
    return { provider: "sw-sapien", available: false, simulated: false, reason: `[PENDIENTE DE CREDENCIALES] faltan: ${this.credentials.missing.join(", ")}` };
  }
