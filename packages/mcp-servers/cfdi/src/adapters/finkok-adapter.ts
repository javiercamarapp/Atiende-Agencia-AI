/**
 * Adaptador real contra Finkok (PAC primario de ejemplo). Esqueleto honesto: ruta
 * SOAP/REST documentada públicamente, auth con usuario/contraseña + CSD del hotel,
 * reintentos con backoff. Sin CSD/credenciales, se declara `unavailable` — NUNCA
 * fabrica un timbrado ni llama a la red sin credenciales verificadas. Puerto
 * ~literal de `hoteles/packages/mcp-servers/cfdi/src/adapters/finkok-adapter.ts`.
 *
 ...
 * `FINKOK_WEBHOOK_SECRET`. Sin ellas, este adaptador SIEMPRE lanza
 * `PortUnavailableError` — el/los test(s) de esta suite nunca dependen de que estas
 * variables existan (usan `FakeFinkokAdapter`).
 */
import { PortUnavailableError, WebhookSignatureError, WebhookReplayError, InMemoryIdempotencyStore, InMemoryReplayGuard, verifyHmacSignature, checkEnvCredentials, type AdapterStatus } from "../shared.ts";
import type { CancelarInput, CfdiCancelacion, CfdiPort, CfdiTimbrado, CfdiWebhookEvent, DomainCfdiStatus, TimbrarInput } from "../port.ts";

const REQUIRED_ENV = ["FINKOK_USERNAME", "FINKOK_PASSWORD", "FINKOK_CSD_CERT_PATH", "FINKOK_CSD_KEY_PATH", "FINKOK_CSD_PASSWORD", "FINKOK_WEBHOOK_SECRET"] as const;
...
export class FinkokAdapter implements CfdiPort {
  ...
  status(): AdapterStatus {
    ...
    return { provider: "finkok", available: false, simulated: false, reason: `[PENDIENTE DE CREDENCIALES] faltan: ${this.credentials.missing.join(", ")}` };
  }
