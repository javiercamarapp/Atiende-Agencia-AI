...
/** Botón píldora inline — mismo patrón EXACTO que
 *  `botonPildoraHtml` en los 6 `staff-invite-template.ts` de domain-*. */
export function botonPildoraHtml(url: string, texto: string): string {
  const href = escapeHtml(url);
  return `<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:6px 0 2px 0;"><tr><td bgcolor="#1D4ED8" style="border-radius:999px;"><a href="${href}" style="display:inline-block;padding:12px 26px;font-family:${FUENTE};font-size:14px;font-weight:600;color:#ffffff;text-decoration:none;border-radius:999px;">${escapeHtml(texto)}</a></td></tr></table>`;
}
...
export interface SeccionPlantilla {
  ...
  /** Línea de marca del pie ("atiende · <lo que hace este producto>") — cada
   *  dominio vertical trae la suya (ver domain-hoteles/emails/layout.ts y
   *  equivalentes); el código cross-vertical (magic link) usa el genérico por
   *  defecto. */
  readonly marcaTagline?: string;
...
export function renderCorreo(s: SeccionPlantilla): string {
  ...
  const tagline = s.marcaTagline ?? "atiende · agencia de AI";
  return `<!doctype html>
<html lang="es">
<head>
<meta charset="utf-8">
  ...
  <tr><td align="center" style="padding:44px 16px 36px 16px;">
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="600" style="width:600px;max-width:100%;border-collapse:collapse;">
      ...
      <tr><td align="left" style="padding:26px 6px 0 6px;">
        <p style="margin:0 0 7px 0;font-family:${FUENTE};font-size:11px;line-height:17px;font-weight:600;letter-spacing:0.08em;text-transform:uppercase;color:#5b6b82;">${escapeHtml(tagline)}</p>
